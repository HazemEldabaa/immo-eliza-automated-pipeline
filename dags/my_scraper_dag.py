...
def run_async(ti):
    version_number = increment_counter(
        counter_path
    )  # Increment and use the new version
...
decide_train_or_skip = BranchPythonOperator(
    task_id="decide_train_or_skip", python_callable=should_run_train, dag=dag
)
...
git_commands = f"""
cd /opt/airflow/myrepo/src
git config user.email "eldabaahazem@gmail.com"
git config user.name "HazemEldabaa"
git pull origin main
git add artifacts_airflow_v{model_version}.joblib
git commit -m 'Automatic update from a CI/CD pipeline - version {model_version}'
git push https://HazemEldabaa:${{GITHUB_TOKEN}}@github.com/HazemEldabaa/immo-eliza-deployment.git main
"""

push_model_task = BashOperator(
    task_id="push_model_to_github",
    bash_command=git_commands,
    env={"GITHUB_TOKEN": Variable.get("GITHUB_TOKEN")},
    dag=dag,
)

run_scraper_task >> run_clean_task >> decide_train_or_skip

decide_train_or_skip >> run_train_task >> push_model_task
