...
class ImmoCrawler:
    ...
    def __init__(self) -> None:
        ...
        self.filters_url = (
            "/province?countries=BE&isALifeAnnuitySale=false&orderBy=postal_code&page="
        )
    ...
    async def get_html(self, url):
        ...
        try:
            ...
            if response.ok:
                print(f"Successfully fetched {url} (Length: {len(response.text)})")
        ...
        async with semaphore:
            ...
            try:
                ...
                for property in properties:
                    ...
                    if (
                        "new-real-estate-project-apartments" in href
                        or "new-real-estate-project-houses" in href
                    ):
                        # Extract links from sub-properties
                        response = await self.get_html(href)
                        # html = await response.text()
                        r = BeautifulSoup(response, "html.parser")
                        sub_properties = r.find_all(
                            "a", attrs={"class": "classified__list-item-link"}
                        )
                        ...
                        for sub_property in sub_properties:
                            ...
                            if sub_href not in self.unique_links:
                                ...
                                self.property_key += 1
                                print(
                                    f"Grabbing Links & Extracting Data: {self.property_key}"
                                )
        ...
        try:
            ...
            if response:
                ...
                json_str = script_content[
                    script_content.find("{") : script_content.rfind("}") + 1
                ]
            ...
            if multi_get(data, "property", "location", "country") == "Belgium":
                ...
                self.property_data[self.property_key] = {
                    ...
                    "construction_year": multi_get(
                        data, "property", "building", "constructionYear"
                    ),
                    "Number_of_frontages": multi_get(
                        data, "property", "building", "facadeCount"
                    ),
                    ...
                    "furnished": (
                        1
                        if multi_get(data, "transaction", "sale", "isFurnished")
                        else 0
                    ),
                    "open_fire": (
                        1 if multi_get(data, "property", "fireplaceExists") else 0
                    ),
                    "terrace_sqm": (
                        multi_get(data, "property", "terraceSurface")
                        if data["property"]["hasTerrace"]
                        else 0
                    ),
                    "terrace": 1 if data["property"]["hasTerrace"] else 0,
                    "garden_sqm": (
                        multi_get(data, "property", "gardenSurface")
                        if data["property"]["hasGarden"]
                        else 0
                    ),
                    ...
                    "primaryEnergyConsumptionPerSqm": multi_get(
                        data,
                        "transaction",
                        "certificates",
